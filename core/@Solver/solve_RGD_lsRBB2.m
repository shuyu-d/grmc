function [x, info, rperi] = solve_RGD_lsRBB2(self, method_init, varargin)
...
%     x = xk + stepsize*desc_dir;
    ...
    grad_transp = self.problem.manifold.transp(xk, x, gradk);

    Y = self.problem.manifold.lincomb(x, 1, grad, -1, grad_transp);
    % Compute the transported step
    S = self.problem.manifold.lincomb(xk, -stepsize, grad_transp);  
